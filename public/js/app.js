...
jQuery.fn.extend({
    CreateMultiCheckBox: function (options) {
        ...
        var detail = $(
            "<div class='MultiCheckBoxDetail'><div class='MultiCheckBoxDetailHeader'><input type='checkbox' class='mulinput' value='-1982' /><div>Chọn tất cả</div></div><div class='MultiCheckBoxDetailBody'></div></div>"
        ).insertAfter(divSel);
        detail.css({
        ...
        this.find("option").each(function () {
            ...
            if (selected) arrSelected.push(val);

            multiCheckBoxDetailBody.append(
                `<div class='cont'><div><input type='checkbox' ${
                    selected ? "checked" : ""
                } class='mulinput' value="${val}" /></div><div>
                    ${$(this).text()} 
                    </div></div>`
            );
        });
        ...
        detail.next().val(arrSelected);
        if (numOption === arrSelected.length)
            detail
                .find(".MultiCheckBoxDetailHeader input")
                .prop("checked", true);
    },
